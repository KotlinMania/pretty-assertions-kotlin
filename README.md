...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`pretty_assertions`](https://crates.io/crates/pretty_assertions).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `pretty_assertions` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Colored, multi-line `assert_eq!`
...
Original work copyrighted by the upstream `pretty_assertions` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.

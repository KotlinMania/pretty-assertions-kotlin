# Claude Code Project Instructions — pretty-assertions-kotlin

## Project Overview

This is **pretty-assertions-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`pretty_assertions`](https://crates.io/crates/pretty_assertions).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/pretty_assertions/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/pretty_assertions>
